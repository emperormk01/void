...
Today is ${today}. Generate a cost report from Void's token usage data.

## Model Pricing (per million tokens)

Use these rates to calculate costs. First check `void.yml` for the `gateway.provider` value.
...
5. **Write the full report** to `articles/cost-report-${today}.md`:
   ...
   *Generated by Void cost-report skill*
   ```
