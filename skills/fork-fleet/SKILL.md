---
name: fork-fleet
description: Inventory active Void forks, detect diverged work, surface upstream contribution candidates
var: ""
tags: [dev]
cron: "0 10 * * 1"
---
...
Today is ${today}. Track Void's fork fleet: discover active forks, analyze divergence, surface the most innovative work for potential upstream contribution.

## Steps

0. **Load managed instance registry** — read `memory/instances.json` (if it exists).
   Build a set of repo full_names that are managed instances (e.g. `owner/void-crypto-tracker`).
...
3. **For each active fork**, analyze divergence by comparing commits against upstream:
   ...
   gh api repos/FORK_OWNER/void/commits --paginate --jq '[.[] | {sha: .sha[0:7], message: .commit.message, author: .commit.author.name, date: .commit.author.date}]' -X GET -f since="$(date -u -d '90 days ago' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-90d +%Y-%m-%dT%H:%M:%SZ)"
   ```
...
4. **Classify divergence** for each fork using the unique commits. Check which areas were modified by fetching the file tree comparison:
   ```bash
   gh api repos/FORK_OWNER/void/compare/${PARENT_REPO##*/}:main...FORK_OWNER:main --jq '{ahead_by: .ahead_by, behind_by: .behind_by, files: [.files[] | {filename, status, additions, deletions}]}'
   ```
   Classify each fork into divergence signals:
   - **New skills**: files added under `skills/` not present in upstream
   - **Custom schedule**: modifications to `void.yml` (different cron times, new skill entries)
...
5. **Score each fork** by divergence:
   ...
   - +1 point per modified core file (void.yml, dashboard, notify)
   - +1 point per star
...
6. **Deep-read top 3 forks**: for each of the top 3 by score, read the actual content of their unique skill files:
   ```bash
   gh api repos/FORK_OWNER/void/contents/skills/SKILL_NAME/SKILL.md --jq '.content' | base64 -d
   ```
...
7. **Write the article** to `articles/fork-fleet-${today}.md`:
   ...
   ## Overview
   Void has N total forks. X are active (pushed within 30 days). Summary of divergence landscape.

   ---

   ## Active Forks — Ranked by Divergence

   ### 1. owner/void — Score: N [MANAGED INSTANCE | COMMUNITY FORK]
   ...
   **Upstream potential:** [Yes — N new skills worth reviewing / No — schedule changes only]
   ...
   ### 2. owner/void — Score: N [MANAGED INSTANCE | COMMUNITY FORK]
   ...
...
8. **Save to memory** — append to `memory/logs/${today}.md`:
   ...
   - Top fork: owner/void (score: N, N unique commits, N new skills)
...
9. **Send a detailed notification** via `./notify`:
   ...
   *Fork Fleet Report — ${today}*
   ...
   Void has [N] active forks (out of [M] total). [1-2 sentence overview of what the fleet looks like — are they mostly dormant clones, or are people actually building?]
   ...
   Upstream contribution candidate: [owner/void]
