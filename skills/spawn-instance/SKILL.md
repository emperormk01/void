---
name: Spawn Instance
description: Clone this Void agent into a new GitHub repo — fork, configure skills, register in fleet
var: ""
tags: [dev]
---
...
Today is ${today}. Create a new Void instance by forking this repo, configuring it for a specific purpose, and registering it in the fleet.
...
1. **Parse the var** to extract instance name and purpose:
   ...
   - Derive the repo name: `void-{name}` (e.g. `void-crypto-tracker`)
...
2. **Check the fleet registry** — read `memory/instances.json` (create if missing).
   ...
   ```json
   {
     "instances": [
       {
         "name": "crypto-tracker",
         "repo": "OWNER/void-crypto-tracker",
         "purpose": "monitor DeFi protocols and token movements",
         "created": "2026-04-08",
         "status": "pending_secrets",
          "skills_enabled": ["token-movers", "token-alert", "heartbeat"],
         "parent": "OWNER/void"
       }
     ]
   }
...
4. **Fork the repo** into the owner's account with the custom name:
   ...
   gh repo fork "$PARENT_REPO" --fork-name "void-{name}" --clone=false
   ```
   ...
   ```bash
   gh api repos/${PARENT_REPO}/forks -X POST -f name="void-{name}"
   ```

5. **Wait for fork availability** — GitHub forks are async. Poll until ready:
   ```bash
   for i in 1 2 3 4 5; do
     gh api "repos/${OWNER}/void-{name}" --jq '.full_name' 2>/dev/null && break
     sleep 5
   done
   ```
...
6. **Select skills for the instance** based on its purpose. Analyze the purpose string and choose from the available skills. Use these heuristics:
   - **crypto/token** purpose: enable `token-movers`, `token-alert`, `monitor-polymarket`, `heartbeat`
   - **research/academic** purpose: enable `deep-research`, `last30`, `heartbeat`
   - **social/twitter/x** purpose: enable `fetch-tweets`, `write-tweet`, `heartbeat`
   - **news/digest** purpose: enable `hacker-news-digest`, `rss-digest`, `weekly-shiplog`, `heartbeat`
   - **dev/code/github** purpose: enable `github-monitor`, `github-trending`, `pr-review`, `heartbeat`
   - **general** or unclear: enable `heartbeat`, `self-improve`
...
7. **Clone the fork, configure it, and push**:
   ...
   TMPDIR=$(mktemp -d)
   gh repo clone "${OWNER}/void-{name}" "$TMPDIR/repo"
   cd "$TMPDIR/repo"
   git config user.name "voidframework"
   git config user.email "voidframework@proton.me"
   ```

   a. **Write a customized `void.yml`**: Read the parent's `void.yml` as a base. Enable only the selected skills. Keep all others disabled. Preserve the structure and comments.

   b. **Write `SETUP.md`** to the repo root:
   ```markdown
   # Void Instance Setup

   This is a managed Void instance spawned from [parent-repo].
...
8. **Enable GitHub Actions** on the fork (forks have Actions disabled by default):
   ```bash
   gh api "repos/${OWNER}/void-{name}/actions/permissions" -X PUT -f enabled=true -f allowed_actions=all
   ```
...
10. **Log to memory** — append to `memory/logs/${today}.md`:
    ```
    ## spawn-instance
    - Created new instance: ${OWNER}/void-{name}
    - Purpose: {purpose}
    - Skills enabled: {list}
...
11. **Send notification** via `./notify`:
    ```
    *New Void Instance Created*

    Repo: {OWNER}/void-{name}
    Purpose: {purpose}
    ...
    Quick setup:
    1. Go to https://github.com/{OWNER}/void-{name}/settings/secrets/actions
