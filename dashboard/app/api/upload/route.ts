...
export async function POST(request: Request) {
  try {
    ...
    // Add to void.yml if not already present
    try {
      const config = await getFileContent('void.yml')
      const updated = addSkillToConfig(config.content, skillName)
      if (updated !== config.content) {
        await updateFile('void.yml', updated, config.sha, `chore: add ${skillName} to config`)
      }
    } catch {
