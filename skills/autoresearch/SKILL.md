...
- Follow Void skill conventions (read memory, log to memory/logs/${today}.md, notify via `./notify`)
...
| **Conventions** | Does it follow Void patterns? (memory, logging, notify, var usage) |
...
- Do not add env vars that aren't already available in the workflow (check void.yml secrets).
