...
export interface GatewayConfig {
  provider: 'openai' | 'anthropic' | 'byok'
}
...
/**
 * Parse void.yml into a typed config object.
 */
export function parseConfig(raw: string): VoidConfig {
  ...
  let gateway: GatewayConfig = { provider: 'openai' }
  const gatewayNode = doc.get('gateway')
  if (isMap(gatewayNode)) {
    const provider = String(getMapValue(gatewayNode, 'provider') ?? 'openai')
    gateway = { provider: provider as GatewayConfig['provider'] }
  }
...
/**
 * Update a skill's config fields in void.yml. Preserves formatting and comments.
 */
...
/**
 * Remove a skill entry from void.yml.
 */
...
/**
 * Add a new skill entry to void.yml (before the fallback comment).
 */
