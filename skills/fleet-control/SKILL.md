---
name: Fleet Control
description: Monitor managed Void instances — check health, dispatch skills, aggregate status
var: ""
tags: [dev]
cron: "0 9,15 * * *"
---
...
Today is ${today}. Monitor and manage the fleet of Void instances registered in `memory/instances.json`.
...
3. **Dispatch the skill** on the child repo:
   ```bash
   gh workflow run void.yml --repo "${REPO}" -f skill="${SKILL_NAME}"
   ```
...
1. **For each instance**, gather:
   ...
   - List of enabled skills (read their void.yml)
   - Recent commits (last 5)

2. **Read each child's void.yml** to see what skills are enabled:
   ```bash
   gh api "repos/${REPO}/contents/void.yml" --jq '.content' | base64 -d
   ```
