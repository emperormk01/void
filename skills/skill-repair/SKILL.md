...
2. **Gather diagnostic context:**
   ...
   - Check recent GitHub Actions runs for the skill:
     ```bash
     gh run list --workflow=void.yml --limit 5 --json status,conclusion,name,createdAt \
       | jq '[.[] | select(.name | contains("SKILL_NAME"))]'
     ```
   - If the run failed, check its logs:
     ```bash
     RUN_ID=$(gh run list --workflow=void.yml --limit 5 --json databaseId,name \
...
4. **Fix it.** Based on diagnosis:
   ...
   - If the skill is fundamentally broken beyond a prompt fix, disable it in `void.yml` and note why

5. **Create a branch and PR:**
   ```bash
   git checkout -b fix/skill-repair-{name}-${today}
   git add skills/{name}/SKILL.md void.yml  # if changed
...
- Never modify secrets or workflow files — only skill files and void.yml.
