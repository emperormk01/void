...
4. **Assess findings.** For each potential vulnerability:
   ...
   **Regardless of whether a vulnerability is confirmed, you MUST document your audit process and findings in the final report.**

5. **Fix the vulnerability (if found).** For each confirmed finding:
...
7. **Push and open a PR to the original repo.**
   ...
   **Location:** \`file:line\`
   ...
   ---
   Found by [Void](https://github.com/voidframework/void) — automated security scanner"
   ```

8. **Write a report.** Save to `articles/vuln-scanner-${today}.md` and `.outputs/vuln-scanner.md`:
   ```markdown
   # Vulnerability Scan — ${today}

   **Repo:** owner/repo (stars, language)
   **Status:** [Clean / Vulnerabilities Found]
   **Findings:** N vulnerabilities/observations

   ## Audit Summary
   [Briefly describe the areas scanned and the approach taken]

   ## Observations / Findings
   [List any suspicious patterns, poor practices, or confirmed vulnerabilities here. If clean, state why it's considered secure.]

   ## Finding 1: [Type]
   - **Severity:** critical/high/medium/info
   - **File:** path:line
   - **Description:** what's wrong
   - **Fix:** what was changed (if any)
   - **PR:** url (if any)
   ```

9. **Notify.** Send via `./notify`:
   ```
   vuln-scanner: [repo] — [Found N vulnerabilities / Audit Clean]. Summary: [one sentence summary]
   PR: [url or N/A]
   ```
...
- Be thorough. Even if no critical bug is found, report the "security posture" of the repo.
