...
export async function PATCH(request: Request) {
  try {
    ...
    const { content, sha } = await getFileContent('void.yml')
    let updated = content
    ...
    if (updated !== content) {
      ...
      await updateFile('void.yml', updated, sha, msg)
    }
...
export async function DELETE(request: Request) {
  try {
    ...
    try {
      ...
      if (updated !== content) {
        await updateFile('void.yml', updated, sha, `chore: remove ${name} from config`)
      }
